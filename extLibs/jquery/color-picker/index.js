(function(window, $, undefined){
	...
	plugin.input = {
		...
		cssAddon: // could also be in a css file instead
			...
			'.cp-panel input {width:28px; height:18px; padding:2px 3px 1px;' +
			...
			'.cp-panel .cp-HEX {width:50px; position:absolute; right:2px;}' +
		...
		renderCallback: function($elm, toggled) {
			...
			$('body').on('click', '.color-picker-input', function(e) {
	...
	plugin.dev_skinned = {
		...
		renderCallback: function($elm, toggled) {
			...
			if (toggled === true) {
				...
				$('.color-picker-input').removeClass('active');
				$elm.closest('.color-picker-input').addClass('active');
			} else if (toggled === false) {
				$elm.closest('.color-picker-input').removeClass('active');
			}
		}
	};
	...
	window.myColorPicker = $('.color-picker-input').colorPicker(
		plugin[type] || plugin.desktop
	);
	
	//$('.color-picker-input').colorPicker();
	//$('.color-picker-input').colorPicker({doRender: false});
