...
(function($) {
            ...
            return this.each(function() {
                ...
                $.each(sliderData, function(i, item) {
                    $('.' + item.combo).change(function() {
                        var val = $(this).val();
                        $('.' + item.slider + '.' + item.combo).slider('option', 'value', val);
                    });
                });                
    ...
    $.fn.createSliderWrap = function(data) {
        ...
        $.each(data, function(i, item) {
            html += '   <div><label>' + item.label + ':</label> <p class="' + item.slider + ' ' + item.combo + '"></p></div>';
        });
            html += '</div>';
    ...
    $.fn.createSlider = function(id, maxValue, combo, stepValue, defValue, orientation) {
        ...
        $this.find('.' + id + '.' + combo).slider({
            ...
            slide: function(event, ui) {
                $this.children('select.combo.' + combo).children('option[value="' + ui.value + '"]').attr('selected', 'selected');
            }            
        });
    }
