	...
	$.setUploadifyLB = function(){
		...
		if($('.uploadSingleFile').length){
			...
			$('.listBox .toggleUploadBox').bind('click', function(){
				$(this).parents('.actionBox').next('div.uploadBox').slideToggle();
				$(this).parents('.actionBox').siblings('div.newFolderBox').slideToggle();
				$(this).parents('.innerListBox').css('height','auto');
			});
			...
			$('.uploadSingleFile').bind('click', function(){
				...
				folder = $(this).parents('div.listBox').find('.listBoxHeader').html().split(" ")[2];
			...
			$('#uploadFile').uploadify({
				...
				'fileExt'			: '*.jpg;*.png;*.gif;*.jpeg;*.JPG;*.JPEG;*.svg;*.doc;*.pdf;*.zip;*.m4v;*.mp4;*.wmf;*.mpeg;*.mpg;*.avi;*.ra;*.ram;*.mov;*.qt;*.ogv;*.webm;*.swf;*.flv;*.f4v;*.mp3',
				'folder'			: 'media/files',
				'onSelectOnce'      : function(event, queueID, fileObj) {
										...
										$('.uploadSingleFile').css('display','inline').removeClass('hide').fadeOut(600).fadeIn(1000);
										$(this).parents('.innerListBox').css('height','auto');
									  },
				...
				'onComplete'        : function(event, queueID, fileObj, response, data) {
										if(response != 0) {
											parObj.parents('.mediaList').children('.showListBox').click().click();
										}
									  },
				'sizeLimit'			: sizeLimit,
				'simUploadLimit'	: 5
