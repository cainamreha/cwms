	...
	$.setUploadifyLBDoc = function(){
		...
		if($('.uploadSingleFile').length){
			...
			$('.uploadSingleFile').bind('click', function(){
				...
				$('#uploadFile').uploadifySettings('scriptData', {'type' : fileType,'imgWidth' : imgWidth,'imgHeight' : imgHeight});
			...
			$('#uploadFile').uploadify({
				...
				'scriptData'		: {'type' : 'doc','response' : 'filename'},
				'multi'				: true,
				'auto'				: false,
				'width'				: 250,
				'height'			: 30,
				...
				'onSelectOnce'      : function(event, queueID, fileObj) {
										...
										parObj.uploadifySettings('scriptData', {'type' : 'doc','response' : 'filename'}, true);
