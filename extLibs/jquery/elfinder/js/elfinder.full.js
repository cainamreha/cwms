 ...
 * Version 2.1_n (Nightly: fdcc1d7) (2014-01-29)
 * http://elfinder.org
 * 
...
window.elFinder = function(node, opts) {
	...
	this
		...
		.bind('rm', function(e) {
			...
			play && play != '' && play != 'no' && $(beeper).html('<source src="' + (typeof(cc.httpRoot) != "undefinded" ? cc.httpRoot + '/extLibs/jquery/elfinder' : '.') + '/sounds/rm.wav" type="audio/wav">')[0].play()
		})
	...
	confirm : function(opts) {
		...
		if (opts.all) {
			...
			options.open = function() {
				...
				/* concise wms Anpassungen */
				pane.find('.ui-button:nth-child(1)').addClass("cancel no");
				pane.find('.ui-button:nth-child(2)').addClass("change");
				pane.find('.ui-button:nth-child(3)').addClass("cancel");

				if (width > parseInt(pane.width())) {
					$(this).closest('.elfinder-dialog').width(width+30).css('left', parseInt(parseInt($(this).css('left')) - (width - parseInt(pane.width())) / 2) + 120 + 'px');
				}
			}
		}
 ...
 * @type String
 **/
elFinder.prototype.version = '2.1_n (Nightly: fdcc1d7)';
 ...
 *     width          : 850,        // dialog width, default 850
	...
	this.filter(':not(.ui-dialog-content)').each(function() {
		...
		if (opts.closeOnEscape) {
			$(document).bind('keyup.'+id, function(e) {
				...
				if (e.keyCode == $.ui.keyCode.ESCAPE && dialog.is('.'+clactive) && !opts.isEditor) { // verhindert ungewolltes Schließen via esc
	...
	var self  = this,
		...
		dialog = function(id, file, content) {
			...
			var dfrd = $.Deferred(),
				...
				opts = {
					title   : file.name,
					width   : self.options.dialogWidth || 900,
					top		: 0,
					isEditor : true,
					buttons : {},
					close   : function() { 
				...
				$.each(self.options.editors || [], function(i, editor) {
					...
					&& typeof editor.save == 'function') {
						ta.editor = {
							load     : editor.load,
							save     : editor.save,
							close    : typeof editor.close == 'function' ? editor.close : function() {alert();},
							instance : null
						}
...
elFinder.prototype.commands.help = function() {
	...
	this.exec = function() {
		if (!this.dialog) {
			this.dialog = this.fm.dialog(content, {title : this.title, width : 530, top : 0, autoOpen : false, destroyOnClose : false});
		}
		...
		var fm    = this.fm,
			...
			open = function(file, id) {
				...
				fm.dialog(dialog, {
					...
					beforeClose		: function(){alert("sdf");}
				}).attr('id', id);
				...
				rhandlec.css({ 'cursor': 'move', 'top': 0 });
