...
/**
 * ArticlesConfigElement class
 * 
 * content type => articles
 */
class ArticlesConfigElement extends ConfigElementFactory implements ConfigElements
{

	private $newsCatName		= "";
	public $dbUpdateArticlesCat = "";

	/**
	 * Gibt ein ArticlesConfigElement zurück
	 * 
	 * @access	public
	...
	public function getConfigElement($a_POST)
	{
		...
		$configArr['dbUpdateArticlesCat']	= $this->dbUpdateArticlesCat;
	...
	public function evalElementPost()
	{
		...
		if(isset($this->a_POST[$this->conPrefix])) { // Falls das Formular abgeschickt wurde
			...
			if($this->newsCatName != ContentsEngine::replaceStaText("<{s_text:allgroups}>") && $this->newsCatName != "") { // Falls ein Kategoriename eingegeben wurde, überprüfen ob dieser bereits existiert
				...
				$existCatName = $this->DB->query( "SELECT *  
														FROM `" . DB_TABLE_PREFIX . "articles_categories` 
			...
			elseif($this->newsCatName == "") { // Falls kein Kategoriename eingegeben wurde
				...
				$this->error =  "{s_error:choosegroup}";
			}
			
			elseif($this->newsCatName == "<{s_text:allgroups}>") { // Falls kein Kategoriename eingegeben wurde
	...
	public function makeUpdateStr()
	{
		...
		// Falls Listentyp, Zielseite für die Kategorie (Tabelle articles_categories) übernehmen
		...
		|| $this->params[1] == "catteaser"
			...
			$this->dbUpdateArticlesCat = "`target_page` = $newsTarget" . $dbFilter;
		}
	...
	public function setParams()
	{
		...
		if($this->params[0] == "<all>")
			$this->newsCatName = "<{s_text:allgroups}>";							
	...
	public function getCreateElementHtml()
	{
		...
		// Button goto articles
		$btnDefs	= array(	"href"		=> ADMIN_HTTP_ROOT . '?task=modules&type=articles&list_cat=',
								"class"		=> "modLink button-icon-only right",
								"text"		=> " &raquo;",
								"title"		=> "{s_title:gotoarticle}",
								"icon"		=> "articles"
							);
		...
		// Articles cats MediaList-Button
		$mediaListButtonDef		= array(	"class"	 	=> "articles category",
											"type"		=> "articles",
											"url"		=> SYSTEM_HTTP_ROOT . "/access/listPages.php?page=admin&type=articles",
											"value"		=> "{s_button:articleschoose}",
											"icon"		=> "articles"
										);
		
		$output .=	$this->getButtonMediaList($mediaListButtonDef);
		
		$output .=	'<label>{s_label:groupname}</label>' . PHP_EOL;
		...
		if(in_array($this->conPrefix, $this->wrongInput)) { // Falls im Fehlerarray vorhanden Meldung ausgeben
			$output .=	'<span class="notice">' . $this->error . PHP_EOL . 
						'<a href="' . ADMIN_HTTP_ROOT . '?task=modules&type=articles&cat_name=' . htmlspecialchars($this->newsCatName) . '">{s_link:newarticlescat}</a></span>' . PHP_EOL;
		...
		$output	.=	'<div class="optionsBar">' . PHP_EOL . 
					'<div class="left"><label>{s_label:articlestype}</label>' . PHP_EOL . 
					'<select name="' . $this->conPrefix . '_news">' . PHP_EOL . 
					'<option value="list"' . ((isset($this->params[1]) && $this->params[1] == "list") ? " selected=\"selected\"" : "") . '>{s_option:articleslist}</option>' . PHP_EOL . 
					'<option value="catteaser"' . ((isset($this->params[1]) && $this->params[1] == "catteaser") ? " selected=\"selected\"" : "") . '>{s_option:catteaser}</option>' . PHP_EOL . 
					'<option value="latest"' . ((isset($this->params[1]) && $this->params[1] == "latest") ? " selected=\"selected\"" : "") . '>{s_option:latestarticles}</option>' . PHP_EOL . 
					...
					'<option value="articlemenu"' . ((isset($this->params[1]) && $this->params[1] == "articlemenu") ? " selected=\"selected\"" : "") . '>{s_option:menuarticles}</option>' . PHP_EOL . 
					'<option value="archive"' . ((isset($this->params[1]) && $this->params[1] == "archive") ? " selected=\"selected\"" : "") . '>{s_option:archivearticles}</option>' . PHP_EOL . 
					'<option value="calendar"' . ((isset($this->params[1]) && $this->params[1] == "calendar") ? " selected=\"selected\"" : "") . '>{s_option:calendararticles}</option>' . PHP_EOL . 
					'<option value="detail"' . ((isset($this->params[1]) && $this->params[1] == "detail") ? " selected=\"selected\"" : "") . '>{s_option:detailarticles}</option>' . PHP_EOL . 
					'<option value="related"' . ((isset($this->params[1]) && $this->params[1] == "related") ? " selected=\"selected\"" : "") . '>{s_option:relatedarticles}</option>' . PHP_EOL . 
					'<option value="popular"' . ((isset($this->params[1]) && $this->params[1] == "popular") ? " selected=\"selected\"" : "") . '>{s_option:populararticles}</option>' . PHP_EOL . 
					'<option value="rated"' . ((isset($this->params[1]) && $this->params[1] == "rated") ? " selected=\"selected\"" : "") . '>{s_option:ratinglist}</option>' . PHP_EOL . 
					'<option value="random"' . ((isset($this->params[1]) && $this->params[1] == "random") ? " selected=\"selected\"" : "") . '>{s_option:randomarticles}</option>' . PHP_EOL . 
					'</select></div>' . PHP_EOL .
