...
/**
 * NewsConfigElement class
 * 
 * content type => news
 */
class NewsConfigElement extends ConfigElementFactory implements ConfigElements
{

	private $newsCatName		= "";
	public $dbUpdateNewsCat		= "";

	/**
	 * Gibt ein NewsConfigElement zurück
	 * 
	 * @access	public
	...
	public function getConfigElement($a_POST)
	{
		...
		$configArr['dbUpdateNewsCat']		= $this->dbUpdateNewsCat;
	...
	public function makeUpdateStr()
	{
		...
		// Falls Listentyp, Zielseite für die Kategorie (Tabelle news_categories) übernehmen
		...
		|| $this->params[1] == "catteaser"
			...
			$this->dbUpdateNewsCat = "`target_page` = '$newsTarget'" . $dbFilter;
		}
	...
	public function getCreateElementHtml()
	{
		...
		$mediaListButtonDef		= array(	"class"	 	=> "news category",
											"type"		=> "news",
											"url"		=> SYSTEM_HTTP_ROOT . "/access/listPages.php?page=admin&type=news",
											"value"		=> "{s_button:newschoose}",
											"icon"		=> "news"
										);
		...
		if(in_array($this->conPrefix, $this->wrongInput)) { // Falls im Fehlerarray vorhanden Meldung ausgeben
			$output	.=	'<span class="notice">' . $this->error . PHP_EOL . 
						'<a href="' . ADMIN_HTTP_ROOT . '?task=modules&type=news&cat_name=' . htmlspecialchars($this->newsCatName) . '">{s_link:newcat}</a></span>' . PHP_EOL;
		...
		$output	.=	'<div class="optionsBar">' . PHP_EOL . 
					'<div class="left"><label>{s_label:newstype}</label>' . PHP_EOL . 
					'<select name="' . $this->conPrefix . '_news">' . PHP_EOL . 
					'<option value="list"' . ((isset($this->params[1]) && $this->params[1] == "list") ? " selected=\"selected\"" : "") . '>{s_option:newslist}</option>' . PHP_EOL . 
					'<option value="latest"' . ((isset($this->params[1]) && $this->params[1] == "latest") ? " selected=\"selected\"" : "") . '>{s_option:latest}</option>' . PHP_EOL . 
					...
					'<option value="archive"' . ((isset($this->params[1]) && $this->params[1] == "archive") ? " selected=\"selected\"" : "") . '>{s_option:archive}</option>' . PHP_EOL . 
					'<option value="calendar"' . ((isset($this->params[1]) && $this->params[1] == "calendar") ? " selected=\"selected\"" : "") . '>{s_option:calendarnews}</option>' . PHP_EOL . 
					'<option value="detail"' . ((isset($this->params[1]) && $this->params[1] == "detail") ? " selected=\"selected\"" : "") . '>{s_option:detail}</option>' . PHP_EOL . 
					'<option value="related"' . ((isset($this->params[1]) && $this->params[1] == "related") ? " selected=\"selected\"" : "") . '>{s_option:relatednews}</option>' . PHP_EOL . 
					'<option value="popular"' . ((isset($this->params[1]) && $this->params[1] == "popular") ? " selected=\"selected\"" : "") . '>{s_option:popularnews}</option>' . PHP_EOL . 
					'<option value="rated"' . ((isset($this->params[1]) && $this->params[1] == "rated") ? " selected=\"selected\"" : "") . '>{s_option:ratinglist}</option>' . PHP_EOL . 
					'<option value="random"' . ((isset($this->params[1]) && $this->params[1] == "random") ? " selected=\"selected\"" : "") . '>{s_option:random}</option>' . PHP_EOL . 
					'</select></div>' . PHP_EOL .
